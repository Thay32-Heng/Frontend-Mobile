...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  void initState() {
    super.initState();
    final isApprover =
        widget.role == Role.headOfDepartment || widget.role == Role.lecturer;
    _tab = isApprover ? "inbox" : "notifications";
    _requests = widget.role == Role.headOfDepartment
        ? List.from(hodRequestsData)
        : List.from(lecturerRequestsData);
  }
    ...
    if (decision == RequestStatus.approved) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(isHoD
              ? "$id approved"
              : "$id recommended for approval — forwarded to HoD"),
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        subtitle,
                        style: TextStyle(
                            fontSize: 12, color: subtitleColor, height: 1.35),
                      ),
                    ],
                  ),
                ),
                    ...
                    Expanded(
                      child: InkWell(
                        ...
                        child: Container(
                          padding: const EdgeInsets.symmetric(vertical: 10),
                          decoration: BoxDecoration(
                            color: _tab == "notifications"
                                ? (isDark
                                    ? const Color(0xFF2E2657)
                                    : Colors.white)
                                : Colors.transparent,
                            borderRadius: BorderRadius.circular(12),
                            boxShadow: _tab == "notifications"
                                ? [
                                    const BoxShadow(
                                        color: Colors.black12, blurRadius: 4)
                                  ]
                      ...
                      child: Column(
                        children: [
                          const Icon(Icons.check_circle_outline,
                              size: 48, color: Color(0xFF05CD99)),
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                children: [
                                  CircleAvatar(
                                    ...
                                    child: Text(getInitials(req.user),
                                        style: const TextStyle(
                                      ...
                                      children: [
                                        Text(req.user,
                                            style: TextStyle(
                                                ...
                                                color: titleColor)),
                                        Text(
                                            "${req.userRole} · submitted ${req.submittedAt}",
                                            style: TextStyle(
                                                fontSize: 10,
                                                color: subtitleColor)),
                                      ],
                              ...
                              const SizedBox(height: 10),
                              Text(
                                  "Dates: ${req.from}${req.to != req.from ? ' → ' + req.to : ''}",
                                  style: TextStyle(
                                ...
                                Row(
                                  children: [
                                    const Icon(Icons.check_circle,
                                        size: 12, color: Color(0xFF05CD99)),
                                    const SizedBox(width: 4),
                                    Text("Screened by: ${req.recommender}",
                                        style: TextStyle(
                                            fontSize: 10,
                                            color: subtitleColor)),
                                  ],
                                ...
                                children: [
                                  Expanded(
                                    child: ElevatedButton.icon(
                                      ...
                                      label: Text(
                                          widget.role == Role.headOfDepartment
                                              ? "Approve"
                                              : "Recommend"),
                                      style: ElevatedButton.styleFrom(
                                        backgroundColor:
                                            const Color(0xFF5B32E8),
                                        shape: RoundedRectangleBorder(
                                            borderRadius:
                                  ...
                                  Expanded(
                                    child: OutlinedButton.icon(
                                      icon: const Icon(Icons.close,
                                          size: 16, color: Color(0xFFEE5D50)),
                ...
                if (!isApprover || _tab == "notifications") ...[
                  if (isSuperAdmin)
                    _buildNotificationCard(
                      ...
                      subtitle:
                          "2 leave + 2 permission requests await your review. Go to Home → Pending Requests.",
                      time: "today",
                      isUnread: true,
                      cardColor: cardColor,
