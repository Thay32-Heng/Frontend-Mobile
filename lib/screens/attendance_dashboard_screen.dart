...
class AttendanceDashboardScreen extends StatefulWidget {
  ...
  const AttendanceDashboardScreen({Key? key, required this.role})
      : super(key: key);
...
class _AttendanceDashboardScreenState extends State<AttendanceDashboardScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final List<UserHistoryItem> allUsers = isSuperAdmin
        ? stakeholderRoles
            .expand((r) => usersByRoleData[r] ?? <UserHistoryItem>[])
            .toList()
        : [
            ...(usersByRoleData["Student"] ?? <UserHistoryItem>[]),
            ...(usersByRoleData["Lecturer"] ?? <UserHistoryItem>[])
          ];

    final total = allUsers.length;
    final presentUsers = allUsers
        .where((u) => u.records
            .any((r) => r.date == "2026-05-29" && r.status == "Present"))
        .toList();
    final lateUsers = allUsers
        .where((u) =>
            u.records.any((r) => r.date == "2026-05-29" && r.status == "Late"))
        .toList();
    final absentUsers = allUsers
        .where((u) => u.records
            .any((r) => r.date == "2026-05-29" && r.status == "Absent"))
        .toList();
    final leaveUsers = allUsers
        .where((u) =>
            u.records.any((r) => r.date == "2026-05-29" && r.status == "Leave"))
        .toList();
          ...
          Row(
            children: [
              _buildStatCard(
                  ...
                  isDark ? const Color(0xFF1E3A32) : const Color(0xFFE6F9F3),
                  Icons.check_circle_outline_rounded,
                  cardColor,
                  subtitleColor,
                  () =>
                      _showDrillDialog("Present Today", presentUsers, isDark)),
              _buildStatCard(
                  "Late",
                  "${lateUsers.length}",
                  const Color(0xFFFFB547),
                  isDark ? const Color(0xFF3E321E) : const Color(0xFFFFF8E7),
                  Icons.access_time_rounded,
                  cardColor,
                  subtitleColor,
                  () => _showDrillDialog("Late Today", lateUsers, isDark)),
              _buildStatCard(
                  "Absent",
                  "${absentUsers.length}",
                  const Color(0xFFEE5D50),
                  isDark ? const Color(0xFF3E1F28) : const Color(0xFFFFF0F0),
                  Icons.error_outline_rounded,
                  cardColor,
                  subtitleColor,
                  () => _showDrillDialog("Absent Today", absentUsers, isDark)),
              _buildStatCard(
                  "Leave",
                  "${leaveUsers.length}",
                  const Color(0xFF63B3ED),
                  isDark ? const Color(0xFF1F2F44) : const Color(0xFFEBF5FF),
                  Icons.calendar_today_rounded,
                  cardColor,
                  subtitleColor,
                  () => _showDrillDialog("On Leave Today", leaveUsers, isDark)),
            ],
          ),
          const SizedBox(height: 16),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      _buildMonthSummaryBox("22", "Days Tracked", "school days",
                          inputBgColor, titleColor, subtitleColor),
                      _buildMonthSummaryBox("87%", "Avg Rate", "attendance",
                          inputBgColor, titleColor, subtitleColor),
                      _buildMonthSummaryBox("3", "Incidents", "alerts flagged",
                          inputBgColor, titleColor, subtitleColor),
                    ],
                  ),
                ],
  ...
  Widget _buildStatCard(String label, String value, Color color, Color bg,
      IconData icon, Color cardColor, Color subtitleColor, VoidCallback onTap) {
    return Expanded(
      child: Padding(
  ...
  Widget _buildMonthSummaryBox(String val, String title, String sub,
      Color inputBgColor, Color titleColor, Color subtitleColor) {
    return Expanded(
      child: Container(
